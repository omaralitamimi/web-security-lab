...
function runSearch(q){
  ...
  if(/\bor\s+1\s*=\s*1/i.test(value)){
    ...
    const result=products.filter(p=>(p.name+" "+p.brand+" "+p.cat).toLowerCase().includes(value.toLowerCase()));
...
function login(e){
  ...
  if(user==="student"&&pass==="student123")setResponse(out,"Signed in successfully. Welcome back, Student.","success");
...
function viewOrder(e){
  e.preventDefault();const id=$("#orderId").value.trim(),out=$("#orderResponse");
  if(id==="1002")setResponse(out,'{"id":1002,"owner":"bob","item":"Studio Camera","total":84}\n'+flags.idor,"data");
  else if(id==="1001")setResponse(out,'Order #1001\nOwner: student\nItem: Minimal Everyday Watch\nStatus: Shipped',"data");
  else setResponse(out,"We could not find that order.","error");
}
...
async function uploadReturn(e){
  e.preventDefault();const file=$("#returnFile").files[0],out=$("#uploadResponse");
  ...
  else setResponse(out,`${file.name} uploaded successfully. Return request opened.`,"success");
}
function invoice(e){
...
$("#newsletterForm").addEventListener("submit",e=>{e.preventDefault();e.target.reset();toast("Welcome to OMAR ALTAMIMI — your code is WELCOME15")});
